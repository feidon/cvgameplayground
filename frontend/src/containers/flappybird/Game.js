...
const Game = ({ setPrePare }) => {
  ...
  const [style, setStlye] = useState({
    transform: `translate(0, ${-GameParamters.bird.initialHeight}px)`,
  }); // 控制bird的css，正數為下方，負數為上方
  ...
  const detect = async (detector) => {
    if (
      ...
      if (
        pose[0].keypoints[10].y <= pose[0].keypoints[6].y &&
        pose[0].keypoints[9].y <= pose[0].keypoints[5].y
      ) {
  ...
  useEffect(() => {
    ...
    const startGame = setInterval(() => {
      ...
      if (playRef.current) {
        ...
        if (
          now - pipeRef.current.lastCreateTime >=
          GameParamters.pipings.timeInterval
        ) {
          creatNewPipe(now);
        }
        // 隨時更新pipeRef.list
  ...
  useEffect(() => {
    (async () => {
      if (UserData.scores.hasOwnProperty("POSE_FLAPPY_BIRD")) {
        //玩過 Pose Flappy Bird
        if (score > UserData.scores["POSE_FLAPPY_BIRD"]) {
          setUserData({
            ...UserData,
            scores: { ...UserData.scores, POSE_FLAPPY_BIRD: score },
          });
  ...
  const creatNewPipe = (now) => {
    const pipeLowerShift =
      GameParamters.pipings.shiftRange.y.min +
      (GameParamters.pipings.shiftRange.y.max -
        GameParamters.pipings.shiftRange.y.min) *
        Math.random(); // 下排pipe向上平移量
    const pipeUpperShift =
      GameParamters.heightLimit - pipeLowerShift - GameParamters.pipings.gap; // by 下排pipe向上平移量 + 上排pipe向下平移量 + gap = ground到天空的長度
    const newPipe = {
  ...
  const collisionDetection = () => {
    ...
    pipeList.map((pipe) => {
      // 過濾出bird正在穿越的pipe
      if (
        pipe.x > GameParamters.pipings.interactionX.head &&
        pipe.x < GameParamters.pipings.interactionX.end
      ) {
        const birdBottom = birdRef.current.height;
        const birdTop = birdRef.current.height + GameParamters.bird.height;
        if (
          birdBottom < pipe.topOfPipeLower - 2 ||
          birdTop > pipe.bottomOfPipeUpper + 2
        ) {
            ...
            <div className="scene-flappybird">
              ...
              {pipeList.map((pipe) => (
                <Pipe
                  key={pipe.createTime}
                  x={pipe.x}
                  pipeLowerShift={pipe.pipeLowerShift}
                  pipeUpperShift={pipe.pipeUpperShift}
                />
              ))}
      ...
      <h3 className="loading-status-flappybird">
        {!playing
          ? loading
            ? "-> Loading MoveNet... <-"
            : detecting
            ? "-> MoveNet is ready, plseae enjoy the game ! <-"
